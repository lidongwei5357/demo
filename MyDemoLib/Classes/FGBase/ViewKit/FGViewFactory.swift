...
@objc public class FGViewFactory: NSObject {
    @objc public static func commitButton(withFrame rect: CGRect, title: String?) -> UIButton {
        ...
        let normalImage = UIImage.fg_image(from: FGUIConfiguration.sharedInstance().navBGColor,
                                           size: CGSize(width: button.width, height: button.height))
        let highlightedImage = UIImage.fg_image(from: FGUIConfiguration.sharedInstance().navBGColor.withAlphaComponent(0.8),
                                                size: CGSize(width: button.width, height: button.height))
