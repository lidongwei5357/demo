...
- (void)createView {
    ...
    CGFloat width = (self.width - leftRight_space * 2 - (colum_space) * (_colum - 1))/_colum;
    CGFloat height = (self.height - _topBottomSpace * 2 - _rowSpace * (_row - 1))/_row;
    ...
    for (int i = 0; i < _count; i++) {
        ...
        UILabel * contentLabel = [[UILabel alloc] initWithFrame:CGRectIntegral(CGRectMake(orginX, titleLabel.bottom, width, height/2))];
...
- (void)layoutSubviews {
    ...
    if (!self.leftArray) {
        if (self.width > 0) {
            ...
            for (int i = 0; i < _colum; i++) {
                [leftFlag addObject:@((int)(self.width/_colum)* i)];
            }
    ...
    for (int i = 0; i < _count; i++) {
        ...
        if ((i+1) % _colum != 0) {
            ...
            width = self.width - [_leftArray[j] floatValue];
        }
        
        titleLabel.frame = CGRectIntegral(CGRectMake([_leftArray[j] floatValue], titleLabel.top, width, height/2));
        contentLabel.frame = CGRectIntegral(CGRectMake([_leftArray[j] floatValue], titleLabel.bottom, width, height/2));
    }
}
